...
public class RespuestaController {
	...
	public ResponseEntity<?> crear(@RequestBody Iterable<Respuesta> respuestas){
		...
		Iterable<Respuesta> respuestasDb = servicio.guardarTodo(respuestas);
		return ResponseEntity.status(HttpStatus.CREATED).body(respuestasDb);
	}
	
	
	/*
	 * «alumnodId» y «examenId» debe ser exactamente a los parametros que pasan por
	 * argumento en el metodo «obtenerRespuestasPorAlumnoPorExamen»; en caso de ser diferentes
	 * se debe usar «@PathVariable(name="nid")»
	 */
	@GetMapping("/alumno/{alumnoId}/examen/{examenId}")
	public ResponseEntity<?> obtenerRespuestasPorAlumnoPorExamen(@PathVariable Long alumnoId, @PathVariable Long examenId){
		
		Iterable<Respuesta> respuestas = servicio.findRespuestaByAlumnoByExamen(alumnoId, examenId);
	...
	public ResponseEntity<?> obtenerExamenesIdsConRespuestasAlumno(@PathVariable Long alumnoId){
		Iterable<Long> examenesIds = servicio.findExamenesIdsConRespuestasByAlumno(alumnoId);
